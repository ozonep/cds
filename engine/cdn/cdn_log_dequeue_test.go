...
func TestStoreNewStepLog(t *testing.T) {
	...
	require.Equal(t, item.StatusItemIncoming, it.Status)
...
func TestStoreLogWrongOrder(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, itemDB)
	require.Equal(t, item.StatusItemIncoming, itemDB.Status)
	require.NotEmpty(t, itemDB.Hash)
	...
	require.Equal(t, item.StatusItemCompleted, itemDB2.Status)
